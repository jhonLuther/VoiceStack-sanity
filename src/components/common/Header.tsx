...
const Header = ({ data }) => {
  ...
  return (
    ...
    {regionSwitcher &&  
      ...
      {regionSwitcherTop &&
          <div className='fixed top-0 left-0 right-0 z-30 flex justify-center bg-white px-4 py-6'>
            <div className='w-full max-w-[1400px] flex gap-16'>
              <p>You are currently viewing our <span>{currentCountry} website</span>. If you want to view this site for another geography, please select from the dropdown.</p>

              <svg onClick={closeRegionPopup} className='absolute right-10 top-10 cursor-pointer' xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 20 20" fill="none">
              ...
              <div className='flex gap-3 items-center'>
                ...
                {/* <button className={headerStyles.header__region__button} onClick={() => handleLocaleContinue(_preferredLocale)}> */}
                  <Button type='primary' className='' onClick={() => _preferredLocale ? goToPreferedLocale(_preferredLocale) : goToPreferedLocale('en')}>
                    Continue
                  </Button>
              </div>
            </div>

...
const TestimonialCard = forwardRef<HTMLDivElement, TestimonialCardProps>(
  ({ data, index, activeIndex, isSticky, scale, onOpenVideo }, ref) => {
    ...
    return (
      <div
        className={`sticky testimonial-card`}
        ref={ref}
        id={`${index}`}
        data-index={index}
        style={{
      ...
      >
        <div
          className={`rounded-[20px] shadow-md max-w-[1032px] mx-auto ${index % 2 === 0 ? 'bg-white' : 'bg-white'}`}
        >
          <div className="flex">
            {/* Image */}
            {data?.image?.url && (
            ...
            <div className="flex-1 flex flex-row justify-between">
              <div className="flex flex-col flex-1 p-8">
                          ...
                          <div className="flex gap-2">
                            {item.before && (
                              <div className="flex gap-2 items-center">
                                <span className="text-gray-900 font-manrope text-[30px] font-bold leading-[1.2]">
                                  {item.before}
                                </span>
                            ...
                            {item.after && (
                              <div className="flex gap-2 items-center">
                                <span className="text-vs-blue font-manrope text-[30px] font-bold leading-[1.2]">
                                  {item.after}
                                </span>
                              </div>
                            )}
                ...
                {data.testimonialdescription && (
                  <p
                    ...
                    dangerouslySetInnerHTML={{
                      __html: data?.testimonialdescription || '',
                    }}
                  ></p>
                )}
              </div>
              ...
              {data.keyFeatures && data.keyFeatures.length > 0 && (
                <div className="flex-1 flex flex-col px-6 pt-12 pb-8 gap-8 border-l border-gray-200 max-w-[270px]">
                  <h3 className="text-gray-500 font-inter text-sm font-medium leading-[110%]">
                    Key Features Used
                  </h3>
                  <ul className="flex flex-wrap gap-3">
                    {data.keyFeatures.map((item: any, index: number) => {
                      return (
                        <li
                          key={index + item}
                          className="flex items-center justify-center px-2 py-[6px] border border-gray-200 text-gray-700 text-sm leading-[1.3] rounded-full"
                        >
                          {item}
                        </li>
                      )
          ...
          <div className="flex flex-row items-center gap-8 border-t border-dashed border-gray-300 py-4 px-8 justify-between">
            ...
            <div>
              <p className="text-base text-gray-900 font-semibold leading-[140%]">
